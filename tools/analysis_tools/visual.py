...
if __name__ == '__main__':
    # nusc = NuScenes(version='v1.0-mini', dataroot='./dataset_4_7_rgb_test_withCar_50_scene', verbose=True)
    # nusc = NuScenes(version='v1.0-mini', dataroot='./dataset_6_19_4_rgb_BEVCA', verbose=True)
    # nusc = NuScenes(version='v1.0-mini', dataroot='./dataset_4_17_rgb_NORMAL', verbose=True)
    # nusc = NuScenes(version='v1.0-mini', dataroot='./dataset_rgb_CNCA', verbose=True)
    
    nusc = NuScenes(version='v1.0-mini', dataroot='./BEVCA_distance_angel_40_0', verbose=True)
    # nusc = NuScenes(version='v1.0-mini', dataroot='./dataset_4_13_rgb_test_withCamouflagedCar_50_scene_time_wrong_fixed', verbose=True)
    # nusc = NuScenes(version='v1.0-mini', dataroot='./data/nuscenes', verbose=True)
    # render_annotation('7603b030b42a4b1caa8c443ccc1a7d52')
    
    # results_filepath = 'test/bevformer_base/BEVCA/pts_bbox/results_nusc.json'
    # results_filepath = 'test/bevformer_base/Normal/pts_bbox/results_nusc.json'
    results_filepath = 'test/bevformer_base/BEVCA_dis_angle_40_0/pts_bbox/results_nusc.json'
    # results_filepath = 'test/bevformer_tiny/Sun_Nov__3_20_30_28_2024/pts_bbox/results_nusc.json'
    

    # 使用正则表达式提取日期时间字符串
    match = re.search(r'test/bevformer_base/(.*?)/pts_bbox', results_filepath)
    if match:
    ...
    bevformer_results = mmcv.load(results_filepath)
    
    save_dir = "result/{}".format(datetime_str)
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    
    sample_token_list = list(bevformer_results['results'].keys())
    print(f"Total number of sample tokens in 'sample_token_list': {len(sample_token_list)}")
    ...
    for id in range(0, 40):
        ...
        render_sample_data(sample_token_list[id], pred_data=bevformer_results, out_path=os.path.join(save_dir, str(id)))
