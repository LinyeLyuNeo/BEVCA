...
class NormalizeMultiviewImage(object):
    ...
    def __call__(self, results):
        ...
        for i, img in enumerate(results['img']):
            ...
            # 进行归一化
            normalized_img = mmcv.imnormalize(img, self.mean, self.std, self.to_rgb)
            normalized_imgs.append(normalized_img)
        
        # results['img'] = [mmcv.imnormalize(img[:,:,3], self.mean, self.std, self.to_rgb) for img in results['img']]
